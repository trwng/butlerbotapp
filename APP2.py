...
class ButlerBotApp(QMainWindow):
    ...
    def update_camera(self):
        ...
        if ret:
            ...
            if self.beverage_detection_enabled:
                ...
                for r in results.boxes.data:
                    ...
                    if label.lower() in ["celsius", "dr pepper", "starbucks cold brew"]:
                        self.query_nutrition(label)
                ...
                if vote_counter:
                    ...
                    if best[1] >= 7:
                        self.fetch_and_print_nutrition(label)
    ...
    def fetch_and_print_nutrition(self, food_input):
        ...
        # Build a formatted text string
            nutrition_info = (
                ...
                f"<b>Carbohydrates:</b> {item.get('nf_total_carbohydrate', 'N/A')} g<br>"
                f"<b>Protein:</b> {item.get('nf_protein', 'N/A')} g<br>"
            )

        # Print to console (like before)
            print("\n--- Nutrition Info ---")
            print(nutrition_info.replace("<br>", "\n"))

        # Show in Nutrition tab
            self.nutrition_text.setText(nutrition_info)
            self.show_nutrition()

        except requests.exceptions.RequestException as e:
            print(f"Network error: {e}")
            self.nutrition_text.setText(f"Network error: {e}")
            self.show_nutrition()
        except (KeyError, IndexError) as e:
            print("Unexpected API response format or missing data.")
            self.nutrition_text.setText("Unexpected API response format or missing data.")
            self.show_nutrition()
